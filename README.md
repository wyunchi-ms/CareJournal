...
OCR 队列在切换应用页面后会继续运行。如果操作系统终止应用，任务会暂停，并在下次打开应用时从未完成项继续。
...
该命令会启动 Vite 开发服务和 Windows Tauri 窗口。项目不再提供独立浏览器版或 Node 服务端。

## LLM 配置

设置页先选择服务商，再为该服务商填写一个 API Key、API 地址和模型。每个服务商只绑定一个模型，切换服务商时会保留各自的本机配置。

Azure OpenAI 使用 v1 API，API 地址填写 `https://<resource>.services.ai.azure.com/openai/v1` 或 `https://<resource>.openai.azure.com/openai/v1`，模型字段填写 Deployment Name，不再需要 API Version。Android、iOS、HarmonyOS 和 Windows 桌面版均使用原生 HTTP 直连服务商。
...
仓库已包含 Capacitor iOS 工程、SQLite 和私有素材存储 Swift 桥接。由于当前尚未在 macOS/Xcode 中编译和真机验证，iOS 仍属于开发候选，不进入公开 Release；详情见 [iOS 原生支持说明](docs/IOS_NATIVE.md)。
