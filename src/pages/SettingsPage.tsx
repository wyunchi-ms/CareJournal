...
export function SettingsPage() {
  ...
  return <>
    <PageHeader eyebrow="本地与隐私" title="应用设置" description="配置 Azure 识别、外观和家属共享备份。" />
    <div className="settings-layout">
      <section className="settings-section card"><div className="settings-heading"><span className="settings-icon"><KeyRound /></span><div><h2>Azure OpenAI</h2><p>Android 原生直连；Web 通过同源兼容层转发以解决 Azure CORS 限制。</p></div></div><div className="callout warning"><AlertTriangle /><span>API Key 仅保存在当前设备且不会进入备份，但对当前应用运行环境可见。Web 兼容层不会持久化请求、图片或密钥。</span></div><div className="form-grid"><label className="full-width">Endpoint<input type="url" value={form.azure.endpoint} onChange={(e) => updateAzure('endpoint', e.target.value)} placeholder="https://your-resource.openai.azure.com" /></label><label>Deployment Name<input value={form.azure.deployment} onChange={(e) => updateAzure('deployment', e.target.value)} /></label><label>API Version<input value={form.azure.apiVersion} onChange={(e) => updateAzure('apiVersion', e.target.value)} /></label><label className="full-width">API Key<span className="password-input"><input type={showKey ? 'text' : 'password'} value={form.azure.apiKey} onChange={(e) => updateAzure('apiKey', e.target.value)} autoComplete="off" /><button type="button" className="icon-button" onClick={() => setShowKey((value) => !value)} aria-label={showKey ? '隐藏密钥' : '显示密钥'}>{showKey ? <EyeOff /> : <Eye />}</button></span></label><label>失败重试次数<select value={form.azure.maxRetries} onChange={(e) => updateAzure('maxRetries', Number(e.target.value))}><option value={1}>1 次</option><option value={2}>2 次</option><option value={3}>3 次</option><option value={4}>4 次</option><option value={5}>5 次</option></select></label></div>{connectionMessage && <p className={`connection-status ${connectionStatus}`} role="status">{connectionStatus === 'success' && <CheckCircle2 />}{connectionMessage}</p>}<div className="form-actions"><button className="button secondary" onClick={() => void test()} disabled={connectionStatus === 'testing'}>测试连接</button><button className="button primary" onClick={() => void save()}>保存配置</button></div></section>
      ...
      <section className="settings-section card legal-note"><h2>产品边界</h2><p>病程记仅用于记录、整理和可视化用户录入的资料，不提供诊断、治疗建议、风险预测或抑制期预测。识别结果来源于图片内容，用户应以医院原始报告为准。</p></section>
    </div>
  </>
}
