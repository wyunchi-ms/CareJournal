...
export function SettingsPage() {
  const { events, chemotherapyTemplates = [], records, pins, preferences, savePreferences, restoreBackup, deduplicateImagesGlobally, storageLabel } = useApp()
  ...
  async function exportData() { try { const blob = await exportBackup(events, chemotherapyTemplates, records, pins, preferences, backupPassword); downloadBlob(blob, `carejournal-${new Date().toISOString().slice(0, 10)}.carejournal`); setBackupMessage('加密备份已生成；Azure API Key 未包含在备份中。') } catch (error) { setBackupMessage(error instanceof Error ? error.message : '导出失败') } }
  async function restore(file: File | undefined) { if (!file) return; try { const payload = await importBackup(file, backupPassword); if (!confirm(`备份包含 ${payload.events.length} 个事件、${payload.records.length} 份检查和 ${payload.chemotherapyTemplates?.length ?? 0} 个治疗方案，将替换本机现有数据。是否继续？`)) return; await restoreBackup(payload); setBackupMessage('备份恢复成功，Azure API Key 保持当前设备原有配置。') } catch (error) { setBackupMessage(error instanceof Error ? error.message : '导入失败') } }
  async function deduplicateImages() {
    setDeduplicating(true)
    setDeduplicationMessage('正在扫描全部检查记录…')
    try {
      const result = await deduplicateImagesGlobally()
      setDeduplicationMessage(result.imagesRemoved > 0 || result.filesDeleted > 0
        ? `扫描完成：共检查 ${result.recordsScanned} 份记录，更新 ${result.recordsUpdated} 份，移除 ${result.imagesRemoved} 张重复图片，回收 ${result.filesDeleted} 个本地文件。`
        : `扫描完成：已检查 ${result.recordsScanned} 份记录，没有发现重复图片。`)
    } catch (error) {
